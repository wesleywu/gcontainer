...
import (
	...
	"github.com/wesleywu/gcontainer/utils/gconv/internal/structcache"
)
...
func RegisterConverter(fn interface{}) (err error) {
	...
	if fnReflectType.Kind() != reflect.Func ||
		...
		err = fmt.Errorf("parameter must be type of converter function and defined as pattern `func(T1) (T2, error)`, but defined as `%s`",
	...
	if inType.Kind() == reflect.Pointer {
		err = fmt.Errorf("invalid converter function `%s`: invalid input parameter type `%s`, should not be type of pointer",
			fnReflectType.String(), inType.String(),
		)
		return
	}
	if outType.Kind() != reflect.Pointer {
		err = fmt.Errorf("invalid converter function `%s`: invalid output parameter type `%s` should be type of pointer",
	...
	if _, ok = registeredOutTypeMap[outType]; ok {
		err = fmt.Errorf("the converter parameter type `%s` to type `%s` has already been registered",
