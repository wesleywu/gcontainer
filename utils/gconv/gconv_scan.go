...
func Scan(srcValue interface{}, dstPointer interface{}, paramKeyToAttrMap ...map[string]string) (err error) {
	...
	if dstPointer == nil {
		return errors.New(`destination pointer should not be nil`)
	}
	...
	if dstPointerReflectKind != reflect.Ptr {
		if dstPointerReflectValue.CanAddr() {
			...
			return fmt.Errorf(`destination pointer should be type of pointer, but got type: %v`, dstPointerReflectType)
		}
	}
