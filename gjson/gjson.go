...
import (
	...
	"github.com/wesleywu/gcontainer/internal/reflection"
...
func (j *Json) setValue(pattern string, value interface{}, removed bool) error {
	...
	for i := 0; i < length; i++ {
		...
		case []interface{}:
			...
			if err != nil {
				err = fmt.Errorf(`strconv.Atoi failed for string "%s"`, array[i])
				return err
			}
...
func (j *Json) convertValue(value interface{}) (convertedValue interface{}, err error) {
	...
	default:
		...
		case reflect.Struct:
			...
			if empty.IsNil(convertedValue) {
				err = fmt.Errorf(`unsupported value type "%s"`, reflect.TypeOf(value))
			}
			return
