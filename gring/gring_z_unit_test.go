...
func TestRing_Val(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		//元素取值并判断和预设值是否相等
		t.Assert(r.Val().name, "jimmy")
		//从当前位置往后移两个元素
		r.Move(2)
		t.Assert(r.Val().name, "alon")
		//更新元素值
		//测试 value == nil
		r.Set(nil)
		t.Assert(r.Val(), nil)
...
func TestRing_Link(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		r := gring.New[string](3)
		r.Put("1")
		r.Put("2")
		r.Put("3")
		s := gring.New[string](2)
		s.Put("a")
		s.Put("b")
...
func TestRing_Slice(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(array2, []int{0, 4, 5, 1, 2})

		array3 := r.SlicePrev() //[2 1 5 4]
		t.Assert(array3, []int{0, 2, 1, 5, 4})

		s := gring.New[int](ringLen)
		for i := 0; i < ringLen; i++ {
			s.Put(i + 1)
		}
		array4 := s.SlicePrev() // []
		t.Assert(array4, []int{1, 5, 4, 3, 2})
	})
}
