...
func ExampleCache_Set() {
	// Create a cache object,
	// Of course, you can also easily use the gcache package method directly
	c := gcache.New[string, []int]()

	// Set cache without expiration
	c.Set(ctx, "k1", []int{1, 2, 3, 4, 5, 6, 7, 8, 9}, 0)

	// Get cache
	fmt.Println(c.Get(ctx, "k1"))

	// Output:
	// [1 2 3 4 5 6 7 8 9] true <nil>
}
...
func ExampleCache_Update() {
	...
	// Sets batch sets cache with key-value pairs by `data`, which is expired after `duration`.
	// It does not expire if `duration` == 0. It deletes the keys of `data` if `duration` < 0 or given `value` is nil.
	c.SetMap(ctx, map[string]string{"k1": "v1", "k2": "v2", "k3": "v3"}, 0)
	...
	kup1, _, _ := c.Get(ctx, "k1")
	fmt.Println(kup1)
	kup2, _, _ := c.Get(ctx, "k2")
	fmt.Println(kup2)
	kup3, _, _ := c.Get(ctx, "k3")
	fmt.Println(kup3)
...
func ExampleCache_Values() {
	// Create a cache object,
	// Of course, you can also easily use the gcache package method directly
	c := gcache.New[string, map[string]string]()

	// Write value
	c.Set(ctx, "k1", map[string]string{"k1": "v1", "k2": "v2"}, 0)
...
func ExampleCache_Close() {
	...
	// Set Cache
	c.Set(ctx, "k1", "v", 0)
	data, _, _ := c.Get(ctx, "k1")
	fmt.Println(data)

	// Close closes the cache if necessary.
	c.Close(ctx)

	data1, _, _ := c.Get(ctx, "k1")
...
func ExampleCache_Data() {
	...
	c.SetMap(ctx, map[string]string{"k1": "v1"}, 0)
...
func ExampleCache_GetOrSetFunc() {
	...
	c.GetOrSetFunc(ctx, "k1", func() (value string, err error) {
		return "v1", nil
	...
	c.GetOrSetFunc(ctx, "k2", func() (value string, err error) {
		return "", nil
	}, 10000*time.Millisecond)
	v1, _, _ := c.Get(ctx, "k2")
	fmt.Println(v1)
...
func ExampleCache_GetOrSetFuncLock() {
	...
	c.GetOrSetFuncLock(ctx, "k1", func() (value string, err error) {
		return "v1", nil
	}, 0)
	v, _, _ := c.Get(ctx, "k1")
	fmt.Println(v)
